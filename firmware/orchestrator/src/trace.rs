...
static CORE_1_EXECUTOR_ID: AtomicU32 = AtomicU32::new(0);
...
fn get_core(executor_id: u32) -> Core {
    if CORE_0_EXECUTOR_ID.load(Ordering::Relaxed) == executor_id {
        Core::Core0
    } else if CORE_1_EXECUTOR_ID.load(Ordering::Relaxed) == executor_id {
        Core::Core1
    } else {
        panic!("Unknown executor id {}", executor_id);
    }
}
...
unsafe extern "Rust" fn _embassy_trace_task_new(executor_id: u32, task_id: u32) {
    ...
    match get_core(executor_id) {
        Core::Core0 => &TASKS_CORE_0,
        Core::Core1 => &TASKS_CORE_1,
    }
    .lock(|tasks| {
        tasks
            .borrow_mut()
...
pub(crate) fn identify_core_0_executor(executor_id: u32) {
    CORE_0_EXECUTOR_ID.store(executor_id, Ordering::Relaxed);
}

pub(crate) fn identify_core_1_executor(executor_id: u32) {
    CORE_1_EXECUTOR_ID.store(executor_id, Ordering::Relaxed);
}
