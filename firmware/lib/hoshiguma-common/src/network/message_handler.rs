...
use hoshiguma_api::{API_PORT, CobsFramer, Message};
...
pub async fn message_handler_loop<F: AsyncFnMut(Message) -> Message>(
    ...
    socket.set_timeout(Some(Duration::from_millis(1000)));

    'conn: loop {
        debug!("socket {}: listening on TCP {}...", id, API_PORT);
        if let Err(e) = socket.accept(API_PORT).await {
            warn!(
